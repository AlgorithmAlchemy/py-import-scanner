#!/usr/bin/env python3
"""
Пример использования анализатора сложности кода
Демонстрирует анализ сложности файлов и проектов
"""
...
def analyze_current_project():
    """Анализирует сложность текущего проекта"""
    print("АНАЛИЗ ТЕКУЩЕГО ПРОЕКТА")
    print("=" * 50)
    ...
    try:
        # Создаем сервис и анализируем проект
        config = Configuration()
        service = ScanService(config)
        
        print(f"Анализируем проект: {project_dir}")
        report = service.analyze_complexity(project_dir)
